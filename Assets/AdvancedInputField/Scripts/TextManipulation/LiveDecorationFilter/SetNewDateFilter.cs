...
namespace AdvancedInputFieldPlugin
{
	...
	public class SetNewDateFilter : LiveDecorationFilter
	{
		/// <summary>The maximum amount of number characters</summary>
		private const int MAX_NUMBERS = 2;
		...
		public override string ProcessText(string text, int caretPosition)
		{
			...
			for(int i = 0; i < length; i++)
			{
				...
				if(IsNumber(c))
				{
					...
					{
						Debug.LogWarning("There are more than 2 number characters. Please set the character limit to 2 to support the date format 00");
						return null;
					}
		...
		public override int DetermineProcessedCaret(string text, int caretPosition, string processedText)
		{
            ...
            if(string.IsNullOrEmpty(processedText))
            {
                if (!string.IsNullOrEmpty(text))
                {
                    if (text.Length >= 2)
                    {
                        Debug.Log("text.Length 2");
                        return 2;
                    }
                    Debug.Log("text.Length " + text.Length);
                    return text.Length;
                }
                Debug.Log("text.Length 0");
                return 0;
            }
