    ...
    public class EmojiAssetBuilder : ScriptableObject
    {
        ...
        public void Build()
        {
            ...
            for (var i = 0; i < json.Count; i++)
            {
                ...
                var codepoints = unified.Split('-');
                string codepoint;

                switch (codepoints.Length)
                {
                    case 1:
                    case 2 when Regex.IsMatch(codepoints[1], @"FE0\w"): // Variation Selectors. https://en.wikipedia.org/wiki/Variation_Selectors_(Unicode_block)
                        codepoint = codepoints[0];
                        break;
                    default: // https://forum.unity.com/threads/how-to-display-emojis-of-multiple-code-points.1368228/
                        Debug.LogWarning($"Ignoring {charName} ({unified}). Multi-codepoint emojis are not supported in the current version of TMP.");
                        continue;
                }

                var unicode = Convert.ToUInt32(codepoint, 16);
